...
{{ config(
    ...
    post_hook="{% if this.name != 'bz_audit_log' %}INSERT INTO {{ ref('bz_audit_log') }} (source_table, load_timestamp, processed_by, processing_time, status) VALUES ('bz_webinars', CURRENT_TIMESTAMP(), 'dbt_transformation', 1, 'COMPLETED'){% endif %}"
) }}
